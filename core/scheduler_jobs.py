...
def send_reminders_job():
    ...
    events = Event.objects.filter(date__date__in=[one_day_from_now, two_days_from_now, three_days_from_now])
    print(f"[APScheduler] Now: {now}, Checking for events on: {one_day_from_now}, {two_days_from_now}, {three_days_from_now}. Events found: {events.count()}")
    for event in events:
